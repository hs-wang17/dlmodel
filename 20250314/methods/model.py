...
class CrossAttention(nn.Module):
    def __init__(self, dim=512):
        ...
        self.mha = nn.MultiheadAttention(dim, num_heads=8)
        self.norm = nn.LayerNorm(dim)
        
    def forward(self, main, sec):
        q = self.query(main.unsqueeze(0))
        k = self.key(sec.unsqueeze(0))
        v = self.value(sec.unsqueeze(0))
        attn_out, _ = self.mha(q, k, v)
        return self.norm(main + attn_out.squeeze(0))
...
class base_model(nn.Module):
    def __init__(self, output_size=1, drop_out=0.2, mask=None, factor_list=None, seed=1):
        ...
        self.cross_attn1 = CrossAttention()             # 交叉注意力模块
        self.cross_attn2 = CrossAttention()
        self.alpha = nn.Parameter(torch.tensor([0.5]))  # 动态融合权重
        self.merge_layer = nn.Linear(32, output_size)   # 最终合并的输出层
    ...
    def forward(self, x):
        ...
        x_combined = x[:, self.selected_factor]         # 提取所有特征（factor_list中的所有特征）
        # 主要特征的处理
        main = self.input_bn_main(x_main)
        main = F.relu(self.input_linear_main(main))
        # 主要特征和次要特征的结合处理
        sec = self.input_bn_combined(x_combined)
        sec = F.relu(self.input_linear_combined(x_combined))
        # 交叉注意力融合
        fused_main = self.cross_attn1(main, sec)
        fused_sec = self.cross_attn2(sec, main)
        combined = self.alpha * fused_main + (1 - self.alpha) * fused_sec
        # 残差处理
        main_out = self.res_blocks_main(combined)
        sec_out = self.res_blocks_combined(combined)
        # 双流输出
        output_main = self.output_main(main_out)
        output_sec = self.output_combined(sec_out)
        # 最终合并
        merged = torch.cat([output_main, output_sec], dim=1)
