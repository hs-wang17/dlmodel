...
class PyramidFusion(nn.Module):
    ...
    def forward(self, features):
        """
        输入: 不同层级的特征列表 (维度从高到低) [L1:512D, L2:256D, L3:128D, L4:64D]
        输出: 融合后的特征 (64D)
        """
        # 从底层开始融合 (L4作为初始融合特征)
        fused = features[-1]  # L4:64D
        # 逆序融合流程
        for i in range(len(features) - 2, -1, -1):
            # 上采样低层特征
            fused = self.upsample(fused.unsqueeze(2)).squeeze(2)
            # 通道对齐
            ...
            # 注意力门控融合
            if i < len(self.attention_gates):
                gate_input = torch.cat([current_feat, fused], dim=1)
                alpha = self.attention_gates[i](gate_input)
                fused = alpha * current_feat + (1 - alpha) * fused
            else:
                fused = current_feat + fused
        return fused
...
class base_model(nn.Module):
    ...
    def _build_pyramid_architecture(self):                          # 构建金字塔融合结构
        ...
        self.base_linear = nn.Linear(len(self.selected_factor), 512)
        # 金字塔残差块
        self.res_blocks = nn.ModuleList([
            ResBlock(512, 512),                                     # Level 1
            ResBlock(512, 256),                                     # Level 2
            ResBlock(256, 128),                                     # Level 3
            ResBlock(128, 64)                                       # Level 4
        ])
        # 金字塔融合模块
