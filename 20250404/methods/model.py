...
class base_model(nn.Module):
    def __init__(self, output_size=1, drop_out=0.2, mask=None, factor_list=None, factor_selection_list=None, seed=1):
        ...
        # 主要特征和次要特征结合的处理
        self.high_imp_mask = torch.tensor([i in factor_selection_list for i in self.selected_factor.cpu().numpy()], dtype=torch.bool, device=ic_mask.device)
        high_dim = self.high_imp_mask.sum().item()
        other_dim = len(self.selected_factor) - high_dim
        self.high_imp_linear = nn.Linear(high_dim, 480)
        self.other_imp_linear = nn.Linear(other_dim, 32)
    ...
    def forward(self, x):
        ...
        x_high_imp = x_combined[:, self.high_imp_mask]
        x_other_imp = x_combined[:, ~self.high_imp_mask]
        x_high_out = F.relu(self.high_imp_linear(x_high_imp))
        x_other_out = F.relu(self.other_imp_linear(x_other_imp))
        x_combined = torch.cat((x_high_out, x_other_out), dim=1)
...
class SimuTradeCapitalLoss(nn.Module):
    def __init__(self, capital=1.5e8, temperature=1e-8, num_iter=5, overused=False):
...
class ExcessReturnLoss(nn.Module):
    def __init__(self, temperature=0.1, capital=1.5e8, top_k=500):
