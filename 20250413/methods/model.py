...
class base_model(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x[:, self.factor_list.to(x.device)]
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model_list, path_name, model_id):
        if self.verbose:
            self.logger.info(
                f'Validation loss decreased ({self.val_loss_min[model_id]:.6f} --> {val_loss[model_id]:.6f}).  Saving model {model_id:.1f}...')
...
class BalancedKMeansMetric:
    ...
    def fit_predict(self, X):
        ...
        if self.metric == 'euclidean':
            def dist_func(x, c): return np.linalg.norm(x - c, axis=1)
        elif self.metric == 'manhattan':
            def dist_func(x, c): return np.sum(np.abs(x - c), axis=1)
        elif self.metric == 'cosine':
            def dist_func(x, c): return 1 - np.dot(x, c) / (np.linalg.norm(x) * np.linalg.norm(c))
        else:
