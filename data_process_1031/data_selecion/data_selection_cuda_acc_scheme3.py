...
def enhanced_scoring(data, factor_cols, m_percent=0.1):
    ...
    # 权重分配
    weights = {'ic': 0.4, 'ic_vol': -0.2, 'long_ratio': 0.15, 'short_ratio': 0.15, 'turnover': -0.1, 'ret_diff': 0.3}
    # 综合评分（全部转换为numpy处理）
    total_scores = (
        weights['ic'] * cp.asnumpy(ic_values) +
        weights['ic_vol'] * cp.asnumpy(ic_vol) +
        weights['long_ratio'] * long_ratio.cpu().numpy() +
        weights['short_ratio'] * short_ratio.cpu().numpy() +
        weights['turnover'] * cp.asnumpy(turnover) +
        weights['ret_diff'] * ret_diff
    )
    return total_scores
...
# 保存结果
result_df.to_feather('/home/datamake117/data/haris/dataset_new/scheme3_selected_factors.fea')
