...
def train_one_Fold(
    ...
    ** 其他：
    ...
    data_scaled = correlation_df.loc[date_list_train].iloc[train_index, :]
    ...
    factor_list = []
    factor_list.append(torch.from_numpy(np.array([list(range(0, 2790))[i] for i in np.where(clusters==0)[0]])))
    factor_list.append(torch.from_numpy(np.array([list(range(0, 2790))[i] for i in np.where(clusters==1)[0]])))
    factor_list.append(torch.from_numpy(np.array([list(range(0, 2790))[i] for i in np.where(clusters==2)[0]])))
