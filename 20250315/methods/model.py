...
class CrossAttentionFusion(nn.Module):
    ...
    def forward(self, main, sec):
        q = self.query(main).unsqueeze(0)
        k, v = self.key_value(sec).chunk(2, dim=-1)
        k = k.unsqueeze(0)
        v = v.unsqueeze(0)
        attn_out, _ = self.mha(q, k, v)
        return self.norm(main + attn_out.squeeze(0))
...
class base_model(nn.Module):
    ...
    def _configure_dual_path(self):
        ...
        # 交叉注意力融合模块
        self.cross_attn = CrossAttentionFusion(dim=256)
        # 动态门控融合（alpha）
    ...
    def forward(self, x):
        ...
        main_feat = self.main_bn(x_main)
        main_feat = F.relu(self.main_linear(main_feat))
        main_feat = self.main_res(main_feat)
        # 路径2：全部特征处理
        x_full = x[:, self.selected_factor]
        full_feat = self.full_bn(x_full)
        full_feat = F.relu(self.full_linear(full_feat))
        full_feat = self.full_res(full_feat)
        # 特征交互融合
        fused_main = self.cross_attn(main_feat, full_feat)
        fused_full = self.cross_attn(full_feat, main_feat)
        # 动态门控融合
        gate_weights = self.gate(torch.cat([fused_main, fused_full], dim=1))
        final_feat = gate_weights[:, 0:1] * fused_main + gate_weights[:, 1:2] * fused_full
        # 最终输出
        return self.final_block(final_feat).squeeze(-1)
