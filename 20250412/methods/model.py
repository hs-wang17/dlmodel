...
class base_model(nn.Module):
    def __init__(self, output_size=1, drop_out=0.2, mask=None, factor_list=None, seed=1, device=None):
        super(base_model, self).__init__()
        # 嵌入层
        self.device = device
        self.code_num = 1000000
        self.embedding_dim = 64
        self.stock_embedding = nn.Embedding(num_embeddings=self.code_num, embedding_dim=self.embedding_dim).to(self.device)
        self.embedding_ln = nn.LayerNorm(self.embedding_dim).to(self.device)
        # 输入层
        self.filter_layers = nn.Sequential(
            nn.Linear(len(mask) + self.embedding_dim, 256),
            nn.BatchNorm1d(256),
            nn.LeakyReLU()
        ).to(self.device)
        # 隐含层+输出层
    ...
    def forward(self, tsdata):
        tsdata = tsdata.to(self.device)
        x = tsdata[:, 1:]
        stock_code = tsdata[:, 0].long()
        embeddings = self.stock_embedding(stock_code)
        embeddings = self.embedding_ln(embeddings)
        tsdata = torch.cat([x, embeddings], dim=1)
        tsdata = self.filter_layers(tsdata)
        tsdata = self.layers(tsdata)
        return tsdata.squeeze(-1)
