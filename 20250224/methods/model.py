...
class base_model(nn.Module):
    ...
    def forward(self, x):
        ...
        x_secondary = x[:, self.secondary_features] # 提取次要特征（factor_list中的680以后的部分）
        # 主要特征的处理
        ...
        # 主要特征和次要特征的结合处理
        x_combined = torch.cat((x_main, x_secondary), dim=1)  # 合并主要特征和次要特征
        ...
        # 使用merge_layer合并两个输出
        final_output = self.merge_layer(torch.cat((output_main.unsqueeze(-1), output_combined.unsqueeze(-1)), dim=-1))
        return final_output
